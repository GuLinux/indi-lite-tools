...
def add_prompt_step(message):
    ...
    sb.add_function(
        functools.partial(send_buzzer, [{"frequency": 1500, "duration": 0.5}, {"frequency": 0, "duration": 0.2}],
                          duration=buzzer_config['default_duration']))
...
def __on_sequence_item_starting(sequence, item):
    send_event('Shoot', 'Shoot started {}/{}, exposure: {}s, remaining: {}, {}s'
               .format(item + 1, sequence.count, sequence.exposure, sequence.remaining_shots,
                       sequence.remaining_seconds))
...
def __on_sequence_item_ended(sequence, item, file_name):
    send_event('Shoot', 'Shoot finished {}/{}, filename: {}, exposure: {}s, remaining: {}, {}s'
               .format(item + 1, sequence.count, file_name, sequence.exposure, sequence.remaining_shots,
                       sequence.remaining_seconds))
    __send_sequence_item_led(sequence, item)
...
def create_sequence(settings):
    ...
    add_sequence(settings['name'], exposure=settings['exp'], count=settings['count'], start_index=1)
...
def change_settings(settings, frame_type='FRAME_LIGHT'):
    ...
    sb.change_camera_settings(
        ...
        switches={'CCD_VIDEO_FORMAT': {'on': ['ASI_IMG_RAW16']},
                  'CCD_CONTROLS_MODE': {'on': ['AUTO_BandWidth'], 'off': ['AUTO_Gain']}}
    )
